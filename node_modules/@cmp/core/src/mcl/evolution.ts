...
import { MER_MAX_GENERATION } from '../types/mcl';

const log = new Logger('Evolution');

export interface EvolvedParams {
  /** Evolved chunk size in KB */
  chunkSizeKb: number;
  /** Evolved device count */
  deviceCount: number;
  /** Evolved tier mapping (5 bytes) */
  tierMapping: Uint8Array;
  /** Decomposition strategy (from best parent) */
  strategy: number;
  /** Generation of this evolved parameter set */
  generation: number;
  /** MER IDs of parents used */
 ...
 */
export function evolveParams(mers: CMP_MER[]): EvolvedParams | null {
  if (mers.length < 2) {
 ...
 */
export function didOutperform(
  evolvedEfficiency: number,
  parents: CMP_MER[]
): boolean {
  if (parents.length === 0) return true;

  const bestParentEfficiency = Math.max(
    ...parents.map(p => p.performance.executionEfficiency)
  );
 ...
 * @returns EMA value
 */
export function efficiencyEMA(history: number[], alpha: number = 0.3): number {
  if (history.length === 0) return 50; // neutral default
