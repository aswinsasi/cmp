...
import { randomBytes, toHex, shortId, Logger } from '../';
import type { TaskType } from '../types';
import type {
  CMP_MER, MERSummary,
  MER_OFFER_Wire, MER_REQUEST_Wire, MER_TRANSFER_Wire,
  MCLConfig,
} from '../types/mcl';
import { DEFAULT_MCL_CONFIG, MER_MAX_PER_JOIN } from '../types/mcl';
import { MERStore } from './dmm';
import { merToWire, merFromWire, verifyMER } from './mer';
import { BloomFilter } from './bloom';

const log = new Logger('Pollinator');

export interface PollinationStats {
  /** Total MERs offered to other meshes */
  totalOffered: number;
  /** Total MERs accepted by other meshes */
  totalAccepted: number;
  /** Total MERs received from other devices */
  totalReceived: number;
  /** Total MERs rejected (invalid signature, duplicate, etc.) */
  totalRejected: number;
  /** Number of mesh join events with pollination */
  joinEvents: number;
}
...
export class Pollinator {
  ...
  private stats: PollinationStats = {
    totalOffered: 0,
    totalAccepted: 0,
    totalReceived: 0,
    totalRejected: 0,
  ...
  constructor(store: MERStore, config?: Partial<MCLConfig>) {
    this.store = store;
    this.config = { ...DEFAULT_MCL_CONFIG, ...config };
  }
  ...
  buildOffer(
    ...
    const taskTypeSet = new Set<number>();
   ...
   */
  processOffer(
    offer: MER_OFFER_Wire,
    selfMeshId: Uint8Array
  ): MER_REQUEST_Wire | null {
    const needed: number[][] = [];

    for (const summary of offer.summaries) {
      const taskType = summary.taskType as TaskType;
  ...
  buildTransfer(
    ...
    const transferData = new TextEncoder().encode(
      mers.map(m => toHex(m.merId)).join(':')
    );
    const transferSig = sign(hash256(transferData), secretKey);

    return {
      senderId: Array.from(selfMeshId),
      mers: mers.map(m => merToWire(m)),
  ...
  processTransfer(
    ...
    for (const merWire of transfer.mers) {
      const mer = merFromWire(merWire);

      // Verify the MER's own signature (from the original creator)
      if (verifyMER(mer, senderPublicKey)) {
