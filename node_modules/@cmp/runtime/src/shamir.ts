...
import { gfPolyEval, lagrangeInterpolateAt0 } from './gf256';

/** A single share: x-coordinate and share data */
export interface ShamirShare {
  /** Share index (1-based, used as x-coordinate in GF(256)) */
  x: number;
...
export function shamirSplit(
  ...
  // Validate parameters
  if (n < 2 || n > 255) throw new Error(`n must be 2-255, got ${n}`);
  if (k < 2) throw new Error(`k must be >= 2, got ${k}`);
  if (k > n) throw new Error(`k must be <= n, got k=${k} n=${n}`);
  ...
  for (let byteIdx = 0; byteIdx < secretLen; byteIdx++) {
    ...
    for (let i = 0; i < n; i++) {
      shares[i].data[byteIdx] = gfPolyEval(poly, shares[i].x);
    }
  }
...
export function shamirReconstruct(shares: ShamirShare[]): Uint8Array {
  if (shares.length === 0) throw new Error('Need at least 1 share');
  ...
  for (let byteIdx = 0; byteIdx < dataLen; byteIdx++) {
    ...
    result[byteIdx] = lagrangeInterpolateAt0(xs, ys);
  }
 ...
 */
export function shamirVerify(
  shares: ShamirShare[],
  expectedSecret: Uint8Array
): boolean {
  try {
    const reconstructed = shamirReconstruct(shares);
    if (reconstructed.length !== expectedSecret.length) return false;
    for (let i = 0; i < reconstructed.length; i++) {
      if (reconstructed[i] !== expectedSecret[i]) return false;
    }
    return true;
  } catch {
    return false;
 ...
 */
export function sharesToWire(shares: ShamirShare[]): { x: number; data: number[] }[] {
 ...
 */
export function sharesFromWire(wire: { x: number; data: number[] }[]): ShamirShare[] {
