...
import { ISignalingChannel, SignalMessage } from './webrtc-signaling';
...
/** Queued message waiting for backpressure drain */
...
interface PeerConnection {
  ...
  /** Negotiation lock — prevents concurrent offer/answer handling */
  negotiating: boolean;
}
...
/** Per-peer connection stats snapshot */
...
export interface WebRTCTransportConfig {
  ...
  /** Max time to wait for ICE connection (ms) */
  iceTimeoutMs?: number;

  /** Max time to wait for DataChannel open (ms) */
  dcOpenTimeoutMs?: number;

  /** Enable detailed logging */
  debug?: boolean;

  /** Max reconnection attempts per peer (default: 5) */
  maxReconnectAttempts?: number;

  /** Enable periodic RTCStats collection (default: true) */
  collectStats?: boolean;

  /** Stats collection interval in ms (default: 10000) */
  statsIntervalMs?: number;

  /**
   * TURN credential refresh callback.
   * Called when ICE servers need credential rotation.
   * Return updated ICE server configs with fresh credentials.
   */
  refreshIceServers?: () => Promise<RTCIceServer[]>;

  /** Interval for TURN credential refresh in ms (default: 3600000 = 1 hour) */
  iceServerRefreshIntervalMs?: number;
}
...
function getRTC(): {
  ...
  for (const pkg of ['@roamhq/wrtc', 'wrtc', 'werift']) {
    try {
      ...
      if (wrtc.RTCPeerConnection) return wrtc;
    } catch {}
  }
...
export class WebRTCTransport implements ITransport {
  ...
  readonly maxPayloadBytes = 16 * 1024 * 1024; // 16MB (fragmented over DC)
  readonly estimatedBandwidthMbps = 250;

  private running = false;
  private signaling: ISignalingChannel;
  private config: Required<Omit<WebRTCTransportConfig, 'refreshIceServers'>> & {
    refreshIceServers?: () => Promise<RTCIceServer[]>;
  };
  private rtc: ReturnType<typeof getRTC> = null;

  /** Active ICE servers (may be refreshed by TURN credential rotation) */
  private activeIceServers: RTCIceServer[];

  /** Random 8-byte instance ID for self-identification */
  private instanceId: Uint8Array;
  private instanceIdHex: string;
  ...
  private peerStats = new Map<string, WebRTCPeerStats>();
  ...
  async start(): Promise<void> {
    ...
    if (!this.rtc) {
      throw new Error(
        'WebRTC not available. In Node.js, install: npm install @roamhq/wrtc'
      );
    }
    ...
    if (this.config.refreshIceServers) {
      this.iceRefreshTimer = setInterval(
        ...
        this.config.iceServerRefreshIntervalMs
      );
    }
  ...
  async stop(): Promise<void> {
    ...
    if (this.iceRefreshTimer) { clearInterval(this.iceRefreshTimer); this.iceRefreshTimer = null; }
  ...
  async startScanning(): Promise<void> { /* always active via signaling */ }
  ...
  async broadcast(data: Uint8Array): Promise<void> {
    ...
    for (const [peerId, peer] of this.peers) {
      if (peer.dcReady) {
        promises.push(
          this.sendTo(`webrtc:${peerId}`, data).catch((err) => {
  ...
  private async handleSignal(signal: SignalMessage): Promise<void> {
    if (!this.running) return;
    if (signal.from === this.instanceIdHex) return;
  ...
  private handleBeaconSignal(signal: SignalMessage): void {
  ...
  private async createPeerConnection(
    ...
    if (existing && existing.dcReady) return existing;
   ...
   */
  private dcSend(peer: PeerConnection, data: Uint8Array): Promise<void> {
    ...
    if (peer.sendQueueBytes + buf.byteLength > DC_MAX_QUEUE_BYTES) {
      return Promise.reject(new Error(
        `Send queue full (${(peer.sendQueueBytes / 1024 / 1024).toFixed(1)}MB queued)`
      ));
    }
   ...
   */
  private drainSendQueue(peer: PeerConnection): void {
    if (peer.draining || !peer.dc || peer.dc.readyState !== 'open') return;
    peer.draining = true;
   ...
   */
  private rejectSendQueue(peer: PeerConnection, error: Error): void {
   ...
   */
  private async attemptIceRestart(peerId: string, peer: PeerConnection): Promise<void> {
    if (!this.running) return;
    if (!peer.isInitiator) return; // Only initiator triggers restart
  ...
  private handleConnectionFailure(peerId: string, peer: PeerConnection): void {
    if (peer.intentionalClose) return;
   ...
   */
  private scheduleReconnect(peerId: string, previousAttempts: number): void {
    if (!this.running) return;
  ...
  private broadcastBeacon(): void {
    if (!this.signaling.isReady() || !this.beaconData) return;
  ...
  private parseStatsReport(
    peerId: string,
    peer: PeerConnection,
    report: RTCStatsReport
  ): WebRTCPeerStats {
    const stats: WebRTCPeerStats = {
      peerId,
    ...
    report.forEach((entry: any) => {
      ...
      if (entry.type === 'candidate-pair' && entry.state === 'succeeded') {
        ...
        if (entry.bytesSent != null) stats.bytesSent = entry.bytesSent;
        if (entry.bytesReceived != null) stats.bytesReceived = entry.bytesReceived;
        if (entry.packetsSent != null) stats.packetsSent = entry.packetsSent;
        if (entry.packetsReceived != null) stats.packetsReceived = entry.packetsReceived;

        // Resolve candidate types from the report
        report.forEach((c: any) => {
  ...
  private async refreshTurnCredentials(): Promise<void> {
    if (!this.config.refreshIceServers) return;
  ...
  private closePeer(peerId: string, peer: PeerConnection): void {
    try {
      if (peer.iceRestartTimer) { clearTimeout(peer.iceRestartTimer); peer.iceRestartTimer = null; }
      if (peer.reconnectTimer) { clearTimeout(peer.reconnectTimer); peer.reconnectTimer = null; }
      if (peer.dc) {
  ...
  getConnectedPeers(): string[] {
    const result: string[] = [];
    for (const [id, peer] of this.peers) {
      if (peer.dcReady) result.push(`webrtc:${id}`);
    }
    return result;
  }
   ...
   */
  getStats(peerId?: string): WebRTCPeerStats | WebRTCPeerStats[] | null {
    if (peerId) {
      const cleanId = this.extractPeerId(peerId);
   ...
   */
  async collectStatsNow(): Promise<WebRTCPeerStats[]> {
    await this.collectAllStats();
    return [...this.peerStats.values()];
  }

  /** Get the active ICE server configuration */
  getActiveIceServers(): RTCIceServer[] {
  ...
  private extractPeerId(address: string): string {
