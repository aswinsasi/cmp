...
class LANTransport {
    name = 'lan';
    maxPayloadBytes = MAX_TCP_MESSAGE;
    ...
    constructor() {
        ...
        if (this.instanceId[0] === 0xFE)
            this.instanceId[0] = 0x00;
    }
    // ── Lifecycle ──
    async start() {
        if (this.running)
    ...
    async stop() {
        ...
        for (const [, socket] of this.peerConnections)
            socket.destroy();
    ...
    async sendTo(peerAddress, data) {
        ...
        if (data.length > MAX_TCP_MESSAGE)
            throw new Error(`Payload too large: ${data.length}`);
    ...
    async startTCP() {
        return new Promise((resolve, reject) => {
            this.tcpServer = net_1.default.createServer((socket) => this.handleIncomingTCP(socket));
    ...
    async startUDP() {
        return new Promise((resolve) => {
            this.udpSocket = dgram_1.default.createSocket({ type: 'udp4', reuseAddr: true });
    ...
    sendBeacon() {
        if (!this.udpSocket || !this.beaconData)
            return;
    ...
    async connectToPeer(peerAddress) {
        ...
        if (!host || !port)
            throw new Error(`Invalid peer address: ${peerAddress}`);
        return new Promise((resolve, reject) => {
            const socket = net_1.default.createConnection({ host, port, timeout: 5000 }, () => {
                ...
                socket.on('data', (chunk) => this.handleTCPData(peerAddress, chunk));
    ...
    handleIncomingTCP(socket) {
        ...
        socket.on('data', (chunk) => this.handleTCPData(addr, chunk));
        socket.on('close', () => this.cleanupPeer(addr));
        socket.on('error', () => this.cleanupPeer(addr));
    }
    handleTCPData(addr, chunk) {
    ...
    get connectedPeerCount() { return this.peerConnections.size; }
    getConnectedPeers() { return [...this.peerConnections.keys()]; }
    /**
...
//# sourceMappingURL=lan-transport.js.map