...
// ── Transport ──
class BLERNTransport {
    name = 'ble-rn';
    maxPayloadBytes = 65536;
    estimatedBandwidthMbps = 2;
    manager = null;
    running = false;
    scanning = false;
    beaconing = false;
    ...
    async startScanning() {
        if (this.scanning || !this.running || !this.manager)
            return;
        this.scanning = true;
        this.manager.startDeviceScan([CMP_SERVICE_UUID], { allowDuplicates: true }, (error, device) => {
            if (error)
                return;
            if (device)
    ...
    async sendTo(peerAddress, data) {
        ...
        if (data.length <= MAX_FRAG_PAYLOAD) {
            await peer.device.writeCharacteristicWithResponseForService(CMP_SERVICE_UUID, CMP_WRITE_CHAR_UUID, this.toBase64(data));
        }
        else {
            // Fragment
            for (let offset = 0; offset < data.length; offset += MAX_FRAG_PAYLOAD) {
                ...
                await peer.device.writeCharacteristicWithResponseForService(CMP_SERVICE_UUID, CMP_WRITE_CHAR_UUID, this.toBase64(frag));
            }
        }
    }
    ...
    async connectPeer(peer) {
        ...
        connected.monitorCharacteristicForService(CMP_SERVICE_UUID, CMP_NOTIFY_CHAR_UUID, (err, char) => {
            if (err || !char?.value)
                return;
    ...
    handleFragment(peerId, totalLen, offset, payload) {
...
exports.BLERNTransport = BLERNTransport;
//# sourceMappingURL=ble-rn-transport.js.map