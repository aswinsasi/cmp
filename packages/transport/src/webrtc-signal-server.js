...
exports.generateSignalToken = generateSignalToken;
...
function createSignalingServer(options = {}) {
    ...
    // ── Auth ──
    function verifyToken(token, peerId) {
        if (!secret)
        ...
        if (Math.abs(now - timestamp) > tokenExpirySeconds)
            return false;
        ...
        const expectedHmac = crypto_1.default
            ...
            .digest('hex');
        return crypto_1.default.timingSafeEqual(Buffer.from(providedHmac, 'hex'), Buffer.from(expectedHmac, 'hex'));
    }
    // ── Rate Limiting ──
    ...
    // ── HTTP Server ──
    const httpServer = options.server ?? http_1.default.createServer((req, res) => {
        if (req.url === '/health') {
    ...
    wss.on('connection', (ws, req) => {
        ...
        const peer = { id: peerId, ws, ip, connectedAt: Date.now() };
        ...
        ws.on('message', (raw) => {
            ...
            try {
                ...
                if (msgStr.length > maxMessageSize)
                    return;
 ...
 */
function generateSignalToken(secret, peerId) {
...
//# sourceMappingURL=webrtc-signal-server.js.map