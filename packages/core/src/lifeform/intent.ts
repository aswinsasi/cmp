/**
 * CMP v1.4 — Intent System
 * A Lifeform declares machine-verifiable INTENTs — formal promises
 * about what it will achieve. The mesh verifies via probabilistic
 * sampling without re-execution or global consensus.
 *
 * Components:
 *   - IntentRegistry: stores and manages intent declarations
 *   - IntentVerifier: evaluates predicates against CRDT state
 *   - IntentSampler: coordinates probabilistic sampling protocol
 *   - ViolationHandler: executes violation actions (slash, kill, etc.)
 *
 * @module lifeform/intent
 * @author Agent Viscro
 */

import {
  IntentContract,
  IntentPredicate,
  IntentSample,
  IntentEvaluationResult,
  PredicateType,
  ViolationAction,
} from '../types/intent';
import { CRDTState } from './crdt/crdt-state';

function toHex(bytes: Uint8Array): string {
  return Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('');
}

function randomBytes(n: number): Uint8Array {
  const bytes = new Uint8Array(n);
  if (typeof crypto !== 'undefined' && crypto.getRandomValues) {
    crypto.getRandomValues(bytes);
  } else {
    for (let i = 0; i < n; i++) bytes[i] = Math.floor(Math.random() * 256);
  }
  return bytes;
}

// ═══════════════════════════════════════
// Intent Registry
// ═══════════════════════════════════════

export class IntentRegistry {
  /** intentId hex → IntentContract */
  private intents = new Map<string, IntentContract>();
  /** lifeformId hex → Set of intentId hex */
  private byLifeform = new Map<string, Set<string>>();

  /**
   * Declare a new intent.
   */
  declare(intent: IntentContract): boolean {
    const hex = toHex(intent.id);
    if (this.intents.has(hex)) return false;

    this.intents.set(hex, intent);

    const lfHex = toHex(intent.lifeformId);
    if (!this.byLifeform.has(lfHex)) {
      this.byLifeform.set(lfHex, new Set());
    }
    this.byLifeform.get(lfHex)!.add(hex);

    return true;
  }

  /**
   * Revoke an intent.
   * Returns the staked CCU minus a 10% early revocation penalty.
   */
  revoke(intentId: Uint8Array): { revoked: boolean; ccuReturned: number } {
    const hex = toHex(intentId);
    const intent = this.intents.get(hex);
    if (!intent) return { revoked: false, ccuReturned: 0 };

    const penalty = intent.ccuStaked * 0.1;
    const returned = intent.ccuStaked - penalty;

    // Clean up indices
    const lfHex = toHex(intent.lifeformId);
    this.byLifeform.get(lfHex)?.delete(hex);
    this.intents.delete(hex);

    return { revoked: true, ccuReturned: returned };
  }

  /**
   * Get an intent by ID.
   */
  get(intentId: Uint8Array): IntentContract | null {
    return this.intents.get(toHex(intentId)) ?? null;
  }

  /**
   * Get all intents for a Lifeform.
   */
  getForLifeform(lifeformId: Uint8Array): IntentContract[] {
    const lfHex = toHex(lifeformId);
    const ids = this.byLifeform.get(lfHex);
    if (!ids) return [];
    return [...ids].map(id => this.intents.get(id)!).filter(Boolean);
  }

  /**
   * Get all active (non-expired) intents.
   */
  getActive(): IntentContract[] {
    const now = Date.now();
    return [...this.intents.values()].filter(i =>
      i.expiresAt === 0 || i.expiresAt > now
    );
  }

  /**
   * Get intents due for sampling (sampleIntervalMs elapsed since last check).
   */
  getDueForSampling(lastCheckedAt: number): IntentContract[] {
    const now = Date.now();
    return this.getActive().filter(i => {
      const elapsed = now - lastCheckedAt;
      return elapsed >= i.sampleIntervalMs;
    });
  }

  /**
   * Record a violation for an intent.
   * Increments consecutive violation count.
   * @returns true if violation threshold crossed
   */
  recordViolation(intentId: Uint8Array): boolean {
    const intent = this.intents.get(toHex(intentId));
    if (!intent) return false;

    intent.consecutiveViolations++;
    return intent.consecutiveViolations >= intent.violationThreshold;
  }

  /**
   * Reset consecutive violations (intent satisfied again).
   */
  resetViolations(intentId: Uint8Array): void {
    const intent = this.intents.get(toHex(intentId));
    if (intent) intent.consecutiveViolations = 0;
  }

  /**
   * Remove all intents for a Lifeform (on death).
   */
  removeAllFor(lifeformId: Uint8Array): number {
    const lfHex = toHex(lifeformId);
    const ids = this.byLifeform.get(lfHex);
    if (!ids) return 0;

    const count = ids.size;
    for (const id of ids) {
      this.intents.delete(id);
    }
    this.byLifeform.delete(lfHex);
    return count;
  }

  /** Total registered intents */
  get size(): number {
    return this.intents.size;
  }
}

// ═══════════════════════════════════════
// Intent Verifier
// ═══════════════════════════════════════

export class IntentVerifier {
  /**
   * Evaluate a predicate against CRDT state.
   * Returns true if the intent is satisfied.
   */
  evaluate(predicate: IntentPredicate, state: CRDTState): boolean {
    switch (predicate.type) {
      case PredicateType.VALUE_CHECK:
        return this.evaluateValueCheck(predicate, state);
      case PredicateType.FRESHNESS_CHECK:
        return this.evaluateFreshnessCheck(predicate, state);
      case PredicateType.RATE_CHECK:
        return this.evaluateRateCheck(predicate, state);
      case PredicateType.COMPOUND:
        return this.evaluateCompound(predicate, state);
      default:
        return false;
    }
  }

  /**
   * Evaluate a full intent contract against state.
   */
  evaluateIntent(intent: IntentContract, state: CRDTState): IntentEvaluationResult {
    const satisfied = this.evaluate(intent.predicate, state);
    return {
      intentId: intent.id,
      satisfied,
      samplesCollected: 1,
      satisfiedCount: satisfied ? 1 : 0,
      violatedCount: satisfied ? 0 : 1,
      evaluatedAt: Date.now(),
    };
  }

  // ── Predicate evaluators ──

  private evaluateValueCheck(predicate: IntentPredicate, state: CRDTState): boolean {
    const value = state.get(predicate.stateKey);
    if (value === undefined) {
      return predicate.operator === 'not_empty' ? false : false;
    }

    const numValue = typeof value === 'number' ? value : parseFloat(String(value));

    switch (predicate.operator) {
      case 'lt': return numValue < (predicate.value as number);
      case 'gt': return numValue > (predicate.value as number);
      case 'eq': return value === predicate.value || numValue === predicate.value;
      case 'lte': return numValue <= (predicate.value as number);
      case 'gte': return numValue >= (predicate.value as number);
      case 'between': {
        const [lo, hi] = predicate.value as [number, number];
        return numValue >= lo && numValue <= hi;
      }
      case 'contains': {
        if (value instanceof Set) return value.has(predicate.value);
        if (typeof value === 'string') return value.includes(String(predicate.value));
        return false;
      }
      case 'not_empty': {
        if (value instanceof Set) return value.size > 0;
        if (Array.isArray(value)) return value.length > 0;
        return value !== null && value !== undefined && value !== '';
      }
      default: return false;
    }
  }

  private evaluateFreshnessCheck(predicate: IntentPredicate, state: CRDTState): boolean {
    // Check if key was updated within the threshold (ms)
    // Simplified: check if key exists and has a value
    // Real impl would check CRDT timestamp
    const value = state.get(predicate.stateKey);
    return value !== undefined;
  }

  private evaluateRateCheck(predicate: IntentPredicate, state: CRDTState): boolean {
    // Rate check: value change per time window
    // Simplified: check if current value meets the threshold
    // Real impl would compare snapshots over time
    return this.evaluateValueCheck(predicate, state);
  }

  private evaluateCompound(predicate: IntentPredicate, state: CRDTState): boolean {
    if (!predicate.children || predicate.children.length === 0) return true;

    if (predicate.logicOperator === 'and') {
      return predicate.children.every(child => this.evaluate(child, state));
    } else {
      return predicate.children.some(child => this.evaluate(child, state));
    }
  }
}

// ═══════════════════════════════════════
// Intent Sampler
// ═══════════════════════════════════════

export interface SamplingRound {
  intentId: Uint8Array;
  /** Selected peer IDs for this round */
  selectedPeers: string[];
  /** Collected samples */
  samples: IntentSample[];
  /** Round started at */
  startedAt: number;
  /** Status */
  status: 'collecting' | 'evaluating' | 'complete';
  /** Result: majority satisfied? */
  result: boolean | null;
}

export class IntentSampler {
  /** intentId hex → SamplingRound */
  private activeRounds = new Map<string, SamplingRound>();
  /** Total rounds completed */
  private totalRounds = 0;
  private totalViolations = 0;

  /**
   * Start a sampling round for an intent.
   * Selects random peers to sample.
   */
  startRound(intent: IntentContract, availablePeers: string[]): SamplingRound | null {
    if (availablePeers.length === 0) return null;

    const numSamplers = Math.min(intent.samplesPerInterval, availablePeers.length);

    // Random selection without replacement
    const shuffled = [...availablePeers].sort(() => Math.random() - 0.5);
    const selectedPeers = shuffled.slice(0, numSamplers);

    const round: SamplingRound = {
      intentId: intent.id,
      selectedPeers,
      samples: [],
      startedAt: Date.now(),
      status: 'collecting',
      result: null,
    };

    this.activeRounds.set(toHex(intent.id), round);
    return round;
  }

  /**
   * Record a sample response from a peer.
   */
  recordSample(intentId: Uint8Array, sample: IntentSample): void {
    const round = this.activeRounds.get(toHex(intentId));
    if (!round || round.status !== 'collecting') return;

    round.samples.push(sample);

    // Check if all samples collected
    if (round.samples.length >= round.selectedPeers.length) {
      this.evaluateRound(round);
    }
  }

  /**
   * Evaluate a sampling round — majority vote.
   */
  evaluateRound(round: SamplingRound): IntentEvaluationResult {
    round.status = 'evaluating';

    const satisfied = round.samples.filter(s => s.satisfied).length;
    const violated = round.samples.filter(s => !s.satisfied).length;
    const majoritySatisfied = satisfied > violated;

    round.result = majoritySatisfied;
    round.status = 'complete';
    this.totalRounds++;

    if (!majoritySatisfied) this.totalViolations++;

    // Clean up
    this.activeRounds.delete(toHex(round.intentId));

    return {
      intentId: round.intentId,
      satisfied: majoritySatisfied,
      samplesCollected: round.samples.length,
      satisfiedCount: satisfied,
      violatedCount: violated,
      evaluatedAt: Date.now(),
    };
  }

  /**
   * Force-complete a round (timeout).
   */
  forceComplete(intentId: Uint8Array): IntentEvaluationResult | null {
    const round = this.activeRounds.get(toHex(intentId));
    if (!round) return null;
    return this.evaluateRound(round);
  }

  /** Get active round for an intent */
  getActiveRound(intentId: Uint8Array): SamplingRound | null {
    return this.activeRounds.get(toHex(intentId)) ?? null;
  }

  /** Get stats */
  getStats(): { totalRounds: number; totalViolations: number; activeRounds: number } {
    return {
      totalRounds: this.totalRounds,
      totalViolations: this.totalViolations,
      activeRounds: this.activeRounds.size,
    };
  }
}

// ═══════════════════════════════════════
// Violation Handler
// ═══════════════════════════════════════

export interface ViolationRecord {
  intentId: Uint8Array;
  lifeformId: Uint8Array;
  action: ViolationAction;
  ccuSlashed: number;
  beneficiaryId: Uint8Array;
  timestamp: number;
}

export class ViolationHandler {
  private history: ViolationRecord[] = [];

  /** Callback hooks */
  private onSlash: ((lifeformId: Uint8Array, amount: number, beneficiary: Uint8Array) => void) | null = null;
  private onKill: ((lifeformId: Uint8Array, reason: string) => void) | null = null;
  private onForceFission: ((compositeId: Uint8Array) => void) | null = null;
  private onNotify: ((lifeformId: Uint8Array, intentId: Uint8Array) => void) | null = null;

  /** Set violation action callbacks */
  setCallbacks(cbs: {
    onSlash?: typeof this.onSlash;
    onKill?: typeof this.onKill;
    onForceFission?: typeof this.onForceFission;
    onNotify?: typeof this.onNotify;
  }): void {
    if (cbs.onSlash) this.onSlash = cbs.onSlash;
    if (cbs.onKill) this.onKill = cbs.onKill;
    if (cbs.onForceFission) this.onForceFission = cbs.onForceFission;
    if (cbs.onNotify) this.onNotify = cbs.onNotify;
  }

  /**
   * Execute a violation action.
   */
  handleViolation(intent: IntentContract): ViolationRecord {
    const record: ViolationRecord = {
      intentId: intent.id,
      lifeformId: intent.lifeformId,
      action: intent.violationAction,
      ccuSlashed: 0,
      beneficiaryId: intent.beneficiaryId,
      timestamp: Date.now(),
    };

    switch (intent.violationAction) {
      case ViolationAction.NOTIFY:
        if (this.onNotify) this.onNotify(intent.lifeformId, intent.id);
        break;

      case ViolationAction.SLASH:
        record.ccuSlashed = intent.ccuStaked;
        if (this.onSlash) this.onSlash(intent.lifeformId, intent.ccuStaked, intent.beneficiaryId);
        break;

      case ViolationAction.KILL:
        record.ccuSlashed = intent.ccuStaked;
        if (this.onSlash) this.onSlash(intent.lifeformId, intent.ccuStaked, intent.beneficiaryId);
        if (this.onKill) this.onKill(intent.lifeformId, `Intent violation: ${intent.description}`);
        break;

      case ViolationAction.FORCE_FISSION:
        if (this.onForceFission) this.onForceFission(intent.lifeformId);
        break;
    }

    this.history.push(record);
    return record;
  }

  /** Get violation history */
  getHistory(): ViolationRecord[] {
    return [...this.history];
  }

  /** Get violation count for a Lifeform */
  getViolationCount(lifeformId: Uint8Array): number {
    const hex = toHex(lifeformId);
    return this.history.filter(r => toHex(r.lifeformId) === hex).length;
  }
}
