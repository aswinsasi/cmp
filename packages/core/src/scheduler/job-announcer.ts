...
import {
  Job, JobState, JobResult,
  JobMessageType, JobAnnounceWire, JobClaimWire, JobResultWire,
} from './job-types';

const log = new Logger('JobAnnounce');
...
export type BroadcastFn = (msgType: number, payload: any) => void;
export type SendToFn = (deviceId: string, msgType: number, payload: any) => void;

// ─── Config ───

export interface JobAnnouncerConfig {
  /** How often to broadcast pending jobs (ms) */
  announceIntervalMs: number;
  /** This device's mesh ID */
  deviceId: string;
  /** Timeout for claims (ms) — if no claim received, re-announce */
  claimTimeoutMs: number;
}

const DEFAULT_ANNOUNCER_CONFIG: JobAnnouncerConfig = {
...
export class JobAnnouncer {
  private queue: JobQueue;
  private config: JobAnnouncerConfig;
  private broadcastFn: BroadcastFn | null = null;
  private sendToFn: SendToFn | null = null;
  ...
  private pendingClaims = new Map<number, JobClaimWire>();

  constructor(queue: JobQueue, config: Partial<JobAnnouncerConfig> = {}) {
    this.queue = queue;
   ...
   */
  setTransport(broadcast: BroadcastFn, sendTo: SendToFn): void {
  ...
  handleMessage(msgType: number, payload: any): void {
    switch (msgType) {
      case JobMessageType.JOB_ANNOUNCE:
        this.handleAnnounce(payload as JobAnnounceWire);
        break;
      case JobMessageType.JOB_CLAIM:
        this.handleClaim(payload as JobClaimWire);
        break;
      case JobMessageType.JOB_RESULT:
        this.handleResult(payload as JobResultWire);
        break;
    }
  }
   ...
   */
  private announcePendingJobs(): void {
    if (!this.broadcastFn) return;

    const pending = this.queue.list({ state: [JobState.QUEUED, JobState.RETRY] });
    ...
    for (const job of pending) {
      ...
      this.broadcastFn(JobMessageType.JOB_ANNOUNCE, wire);
   ...
   */
  private handleAnnounce(wire: JobAnnounceWire): void {
    // Don't claim our own announcements
    if (wire.announcerId === this.config.deviceId) return;
   ...
   */
  private handleClaim(wire: JobClaimWire): void {
    const job = this.queue.get(wire.jobId);
    if (!job) return;

    // Only accept claims for our jobs that are still pending
    if (job.state !== JobState.QUEUED && job.state !== JobState.RETRY) return;
   ...
   */
  private handleResult(wire: JobResultWire): void {
    const job = this.queue.get(wire.jobId);
    if (!job) return;
