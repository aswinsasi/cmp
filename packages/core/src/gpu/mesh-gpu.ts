...
import { DEFAULT_MESH_GPU_CONFIG, NO_GPU } from '../types/gpu';
import { GPUExecutor } from './executor';
...
export interface GPUPeerInfo {
  deviceId: string;
  capability: GPUCapability;
  latencyMs: number;
}

export interface MeshGPUTransport {
  /** Get local device ID */
  getLocalDeviceId(): string;
  /** Get GPU-capable peers */
  getGPUPeers(): GPUPeerInfo[];
  /** Send a GPU task to a remote device for execution */
  sendGPUTask(deviceId: string, task: GPUTask): Promise<GPUTaskResult>;
}

// ═══════════════════════════════════════

export class MeshGPU {
  private config: MeshGPUConfig;
  private transport: MeshGPUTransport;
  private localExecutor: GPUExecutor;
  ...
  constructor(
    transport: MeshGPUTransport,
    config?: Partial<MeshGPUConfig>,
    localCapability?: Partial<GPUCapability>,
  ) {
    this.config = { ...DEFAULT_MESH_GPU_CONFIG, ...config };
    this.transport = transport;
    this.localExecutor = new GPUExecutor(localCapability);
  }
  ...
  async distributeCompute(
    ...
    data: Float32Array,
    shape: [number, number],
    splitStrategy: 'row' | 'column' | 'block' = 'row',
  ): Promise<{ result: Float32Array; devicesUsed: number; totalMs: number }> {
    ...
    // Execute in parallel
    const promises: Promise<GPUTaskResult>[] = [];

    for (let i = 0; i < chunks.length; i++) {
      const targetDevice = i < gpus.length ? gpus[i].deviceId : 'local';
      const buffers: GPUBufferInput[] = [{
        label: `chunk-${i}`,
        data: chunks[i],
        usage: 'storage',
      }];
      ...
      if (targetDevice === 'local') {
        ...
        promises.push(
          this.transport.sendGPUTask(targetDevice, task).catch(() =>
            this.localExecutor.execute(task) // Fallback
          )
        );
      }
    }
   ...
   */
  async handleRemoteTask(task: GPUTask): Promise<GPUTaskResult> {
   ...
   */
  private selectBestGPU(task: GPUTask): string {
    const peers = this.transport.getGPUPeers().filter(p =>
      p.capability.available &&
   ...
   */
  private getAvailableGPUs(): GPUPeerInfo[] {
    const localId = this.transport.getLocalDeviceId();
    const localCap = this.localExecutor.getCapability();

    const all: GPUPeerInfo[] = [];
    ...
    const remotes = this.transport.getGPUPeers().filter(p =>
      p.capability.available &&
      p.capability.computeUtilization < this.config.maxUtilizationForAcceptance
    );
   ...
   */
  getLocalCapability(): GPUCapability {
   ...
   */
  getMeshCapabilities(): GPUPeerInfo[] {
   ...
   */
  registerKernel(name: string, kernel: (inputs: Float32Array[], workgroups: [number, number, number]) => Float32Array[]): void {
