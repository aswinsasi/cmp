...
const log = new logger_1.Logger('StageExec');
// ─── Stage Executor ───
class StageExecutor {
    stages = new Map();
    /**
     ...
     */
    initStage(definition, deviceId, backpressure, downstream, bufferCapacity = 100) {
        // Resolve the stage function
        let fn;
        if (definition.type === 'builtin') {
            const builtin = (0, builtin_stages_1.getBuiltinStage)(definition.name);
            if (!builtin) {
     ...
     */
    processItem(stageIndex, item) {
        const runtime = this.stages.get(stageIndex);
        if (!runtime || runtime.instance.state !== 'running')
            return [];
        // Check backpressure
