...
exports.GenericPattern = exports.CompressPattern = exports.HashPattern = exports.MLTrainPattern = exports.MatrixPattern = void 0;
...
// ═══════════════════════════════════════
class MatrixPattern {
    name = compiler_types_1.ParallelPattern.MATRIX;
    mergeType = compiler_types_1.MergeType.BLOCK_ASSEMBLE;
    detect(wasmExports, inputData, meta) {
        ...
        return {
            pattern: compiler_types_1.ParallelPattern.MATRIX,
            confidence,
...
// ═══════════════════════════════════════
class MLTrainPattern {
    name = compiler_types_1.ParallelPattern.ML_TRAIN;
    mergeType = compiler_types_1.MergeType.GRADIENT_AVERAGE;
    detect(wasmExports, inputData, meta) {
        let confidence = 0;
        let reason = '';
        ...
        return {
            pattern: compiler_types_1.ParallelPattern.ML_TRAIN,
            confidence,
...
exports.MLTrainPattern = MLTrainPattern;
// ═══════════════════════════════════════
// Hash Pattern
// ═══════════════════════════════════════
class HashPattern {
    name = compiler_types_1.ParallelPattern.HASH;
    ...
    detect(wasmExports, inputData, meta) {
        ...
        return {
            pattern: compiler_types_1.ParallelPattern.HASH,
            confidence,
    ...
    decompose(inputData, chunkCount, meta) {
        ...
        return { pattern: compiler_types_1.ParallelPattern.HASH, chunks, meta: {} };
    }
    merge(results, meta) {
        const startMs = Date.now();
...
// Compress Pattern
// ═══════════════════════════════════════
class CompressPattern {
    name = compiler_types_1.ParallelPattern.COMPRESS;
    mergeType = compiler_types_1.MergeType.CONCATENATE;
    detect(wasmExports, inputData, meta) {
        let confidence = 0;
        let reason = '';
        ...
        return {
            pattern: compiler_types_1.ParallelPattern.COMPRESS,
            confidence,
    ...
    decompose(inputData, chunkCount, meta) {
        ...
        const chunks = splitEvenly(inputData, chunkCount, 1);
        return { pattern: compiler_types_1.ParallelPattern.COMPRESS, chunks, meta: {} };
    }
    merge(results, meta) {
        const startMs = Date.now();
        const totalLen = results.reduce((s, r) => s + r.length, 0);
...
exports.CompressPattern = CompressPattern;
// ═══════════════════════════════════════
// Generic Pattern (replicate and race)
// ═══════════════════════════════════════
class GenericPattern {
    name = compiler_types_1.ParallelPattern.GENERIC;
    mergeType = compiler_types_1.MergeType.RACE_WINNER;
    detect(_wasmExports, _inputData, _meta) {
        // Generic always matches as a fallback with low confidence
        return {
            pattern: compiler_types_1.ParallelPattern.GENERIC,
            confidence: 0.2,
...
//# sourceMappingURL=compute-patterns.js.map