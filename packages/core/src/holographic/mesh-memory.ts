...
import type {
  ...
  ShardDescriptor,
  ShardLocation,
} from '../types/holographic';
import { DEFAULT_MESH_MEMORY_CONFIG } from '../types/holographic';

function toHex(bytes: Uint8Array): string {
  return Array.from(bytes).map(b => b.toString(16).padStart(2, '0')).join('');
}

// ─── Shard Store (local device storage) ───
...
// ─── Peer Interface (injected by CMPNode) ───

export interface MeshMemoryPeer {
  /** Device mesh ID (hex) */
  deviceId: string;
...
export interface MeshMemoryTransport {
  /** Get list of active peers */
  getPeers(): MeshMemoryPeer[];
  /** Get local device ID */
  getLocalDeviceId(): string;
  /** Send shard to a peer for storage */
  sendShard(peerId: string, shard: ShardDescriptor): Promise<boolean>;
  /** Request a shard from a peer */
  requestShard(peerId: string, key: string, shardIndex: number): Promise<ShardDescriptor | null>;
  /** Query which peers hold shards for a key */
  queryShardLocations(key: string): Promise<ShardLocation[]>;
}
...
interface KeyMeta {
  key: string;
  originalSize: number;
  k: number;                  // required shards
  n: number;                  // total shards
  shardSize: number;
  writtenAt: number;
  ttlMs: number;
  /** Map: shardIndex → deviceId hosting it */
...
export class MeshMemory {
  private config: MeshMemoryConfig;
  private transport: MeshMemoryTransport;

  /** Local shard storage: key:shardIndex → shard */
  private localShards = new Map<string, LocalShard>();
  /** Key metadata: key → meta */
  private keyMeta = new Map<string, KeyMeta>();
  /** Total bytes stored locally */
  private localBytes = 0;

  constructor(transport: MeshMemoryTransport, config?: Partial<MeshMemoryConfig>) {
    this.config = { ...DEFAULT_MESH_MEMORY_CONFIG, ...config };
   ...
   */
  async write(key: string, data: Uint8Array, options?: WriteOptions): Promise<number> {
    if (data.length === 0) throw new Error('Cannot write empty data');
    ...
    if (options?.requiredShards && options?.totalShards) {
      ...
      const counts = calculateShardCounts(
        data.length,
        this.config.maxShardSizeBytes,
        this.config.defaultRedundancy,
      );
      k = counts.k;
      m = counts.m;
    }
    ...
    // Erasure encode
    const shards = rsEncode(data, k, m);

    // Build shard descriptors
    const descriptors: ShardDescriptor[] = shards.map((shardData, index) => ({
      key,
      shardIndex: index,
      totalShards: n,
      requiredShards: k,
      data: shardData,
      checksum: simpleHash(shardData),
    ...
    const shardLocations = new Map<number, string>();
    let distributed = 0;
  ...
  async read(key: string): Promise<ReadResult> {
    ...
    if (!meta) throw new Error(`Key not found: ${key}`);
    ...
    // Collect k shards
    const collected: Array<{ index: number; data: Uint8Array }> = [];
    ...
    for (const [shardIndex, deviceId] of meta.shardLocations) {
      if (deviceId === this.transport.getLocalDeviceId()) {
        ...
        if (local) {
          ...
          if (collected.length >= meta.k) break;
        }
      }
    }
    ...
    if (collected.length < meta.k) {
      ...
      for (const [shardIndex, deviceId] of meta.shardLocations) {
        if (collectedIndices.has(shardIndex)) continue;
        if (deviceId === this.transport.getLocalDeviceId()) continue;

        const shard = await this.transport.requestShard(deviceId, key, shardIndex);
        if (shard && verifyShard(shard.data, shard.checksum)) {
    ...
    if (collected.length < meta.k) {
      throw new Error(
        `Insufficient shards for ${key}: need ${meta.k}, got ${collected.length}`
      );
    }

    // Reconstruct
    const data = rsDecode(collected, meta.k, meta.n, meta.originalSize);
   ...
   */
  storeLocalShard(descriptor: ShardDescriptor): boolean {
   ...
   */
  getLocalShard(key: string, shardIndex: number): LocalShard | null {
   ...
   */
  hasLocalShard(key: string, shardIndex: number): boolean {
  ...
  private removeLocalShard(key: string, shardIndex: number): void {
    const storeKey = `${key}:${shardIndex}`;
    const existing = this.localShards.get(storeKey);
    if (existing) {
      this.localBytes -= existing.descriptor.data.length;
  ...
  private evict(neededBytes: number): boolean {
  ...
  private evictLFU(neededBytes: number): boolean {
    const entries = Array.from(this.localShards.entries())
  ...
  private evictTTL(neededBytes: number): boolean {
    const now = Date.now();
    let freed = 0;
   ...
   */
  getKeyMeta(key: string): KeyMeta | undefined {
