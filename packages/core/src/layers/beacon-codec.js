...
exports.isValidBeacon = isValidBeacon;
...
function encodeBeacon(beacon) {
    const buf = new ArrayBuffer(beacon_1.BEACON_SIZE);
    ...
    // Flags (1 byte)
    let flags = 0;
    if (beacon.flags.acceptingTasks)
        flags |= 0x01;
    if (beacon.flags.hasPendingTasks)
        flags |= 0x02;
    if (beacon.flags.relayCapable)
        flags |= 0x04;
    if (beacon.flags.mclCapable)
        flags |= 0x08;
...
function decodeBeacon(data) {
    if (data.length < beacon_1.BEACON_SIZE)
        return null;
    const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
    let offset = 0;
    // Magic (3 bytes)
    const magic = (view.getUint8(offset) << 16) |
        (view.getUint8(offset + 1) << 8) |
        view.getUint8(offset + 2);
    offset += 3;
    if (magic !== beacon_1.BEACON_MAGIC)
        return null;
    // Version (1 byte)
 ...
 */
function createBeacon(meshId, capabilityHash, flags = {}) {
    return {
        magic: beacon_1.BEACON_MAGIC,
        version: beacon_1.PROTOCOL_VERSION,
        meshId,
        capabilityHash,
        timestamp: (0, helpers_1.now)(),
        ttl: beacon_1.DEFAULT_TTL,
        flags: {
            acceptingTasks: true,
...
function isValidBeacon(beacon) {
    if (beacon.magic !== beacon_1.BEACON_MAGIC)
        return false;
    if (beacon.version < 1)
        return false;
    if (beacon.meshId.length !== 16)
        return false;
    if (beacon.capabilityHash.length !== 8)
        return false;
    if (beacon.ttl < 0 || beacon.ttl > 255)
        return false;
    return true;
}
//# sourceMappingURL=beacon-codec.js.map