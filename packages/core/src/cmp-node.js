...
exports.CMPNode = void 0;
exports.generateCertificate = generateCertificate;
exports.verifyCertificate = verifyCertificate;
exports.exportCertificateJSON = exportCertificateJSON;
exports.importCertificateJSON = importCertificateJSON;
const lan_transport_1 = require("../../transport/src/lan-transport");
const multi_transport_1 = require("../../transport/src/multi-transport");
const v2_bridge_1 = require("./v2-bridge");
const v4_wire_handler_1 = require("./v4-wire-handler");
const _1 = require("./");
const src_1 = require("../../runtime/src");
const engine_1 = require("./mcl/engine");
const log = new _1.Logger('CMPNode');
// ── Main Class ──
class CMPNode {
    config;
    transport;
    ...
    lifeformHandler = null;
    /** V2 Bridge: Layers 11-13 (v2.0) */
    v2bridge = null;
    v4WireHandler = new v4_wire_handler_1.V4WireHandler();
    /**
    ...
    constructor(config = {}) {
        if (config.logLevel !== undefined) {
            _1.Logger.setLevel(config.logLevel);
        }
        this.config = (0, _1.resolveConfig)(config);
        this.bus = new _1.EventBus();
        this.peerTable = new _1.PeerTable(this.bus, this.config.peerStaleMs, this.config.peerDeadMs);
        // Transport
        ...
        // Layer 1: Discovery
        this.discovery = new _1.DiscoveryLayer(this.transport, this.bus, this.peerTable, this.config);
        // Layer 2: Capability
        this.profiler = new _1.DeviceProfiler({ maxResourceShare: this.config.maxResourceShare });
        this.capMap = new _1.CapabilityMap(this.bus);
        this.capExchange = new _1.CapabilityExchange(this.discovery.getMeshId(), this.transport, this.bus, this.peerTable, this.capMap, this.profiler, (meshId) => this.discovery.resolveAddress(meshId));
        // Layer 3: Negotiation (needs execution deps, so build those first)
        // Layers 4-6: Distribution, Execution, Assembly
        this.codeCache = new src_1.CodeCache(100);
        this.executionEngine = new src_1.ExecutionEngine(this.discovery.getMeshId(), this.codeCache, { maxConcurrent: this.config.maxConcurrentTasks });
        this.distributor = new src_1.TaskDistributor();
        this.splitter = new src_1.DataSplitter();
        const signingKP = (0, _1.generateSigningKeyPair)();
        this.signingKP = signingKP;
        this.ledger = new _1.IncentiveLedger({
        ...
        this.negotiation = new _1.NegotiationEngine(this.discovery.getMeshId(), signingKP, this.transport, this.bus, this.peerTable, this.capMap, this.discovery, this.ledger, {
        ...
        this.bidHandler = new _1.BidHandler(this.discovery.getMeshId(), this.transport, this.bus, this.profiler, this.discovery, this.executionEngine, this.codeCache, this.ledger, {
        ...
        log.info(`CMPNode created: ${(0, _1.shortId)(this.discovery.getMeshId())}`);
        // Layer 8: Mesh Cognition (v1.2)
        this.mclEngine = new engine_1.MCLEngine({
        ...
        this.v2bridge = new v2_bridge_1.V2Bridge(this);
    }
    // ══════════════════════════════════════════
    // Lifecycle
    // ══════════════════════════════════════════
    ...
    async start() {
        ...
        this.bus.on('chunk:executed', (data) => {
            if (data.status === _1.ChunkStatus.SUCCESS) {
                const myHex = this.meshIdHex();
                const earned = data.creditsEarned || 1;
                this.ledger.earnCredits(myHex, 'mesh', earned, (0, _1.toHex)(data.taskId), (0, _1.toHex)(data.chunkId));
                this.ledger.recordCompletion(myHex, true);
                log.debug(`Earned ${earned} CCU for executing chunk ${(0, _1.shortId)(data.chunkId)}`);
    ...
    async stop() {
        ...
        try {
            ...
            const msg = (0, _1.encodeMessage)(_1.MessageType.DEPARTURE_NOTICE, (0, _1.encodeJSON)(notice));
    ...
    async compute(wasmModule, inputData, options = {}) {
        if (!this.running)
            throw new Error('CMPNode not running');
        ...
        const runtimeCheck = (0, src_1.detectRuntime)(wasmModule);
        if (runtimeCheck !== null) {
            log.warn(`SECURITY: Blocked ${runtimeCheck.runtime} code from mesh distribution. ` +
                `Non-WASM runtimes have no sandbox isolation and MUST NOT execute on remote peers. ` +
                `Falling back to local execution.`);
            return this.executeLocalMultiRuntime(runtimeCheck.runtime, runtimeCheck.code, inputData, deadline);
        }
        // Cache the WASM module
        const moduleHash = this.codeCache.store(wasmModule);
        // Build compute request
        const request = {
            taskType: options.taskType ?? _1.TaskType.MAP_REDUCE,
            runtimeRequired: options.runtime ?? _1.Runtime.WASM,
            ...
            priority: options.priority ?? _1.Priority.NORMAL,
            security: {
                verifyMode: options.certify ? _1.VerifyMode.REDUNDANT : (options.verifyMode ?? _1.VerifyMode.CHECKSUM),
                encryption: _1.EncryptionAlgo.AES_256_GCM,
                dataSensitivity: options.sensitivity ?? _1.SecurityLevel.PRIVATE,
            },
        ...
        const myHex = this.meshIdHex();
        if (!this.ledger.spendCredits(myHex, totalCredits, (0, _1.toHex)(negotiationResult.taskId))) {
            log.warn(`Insufficient credits (${this.ledger.getBalance(myHex)} CCU), executing locally`);
            return this.executeLocally(wasmModule, inputData, entryPoint, startTime);
        }
        log.debug(`Spent ${totalCredits} CCU for task ${(0, _1.shortId)(negotiationResult.taskId)}`);
        // Phase 2: Distribute
        const codeRef = {
            runtime: _1.Runtime.WASM,
            moduleHash,
            entryPoint,
        };
        ...
        const taskRequest = {
            ...
            security: {
                encryption: _1.EncryptionAlgo.AES_256_GCM,
                verifyMode: request.security?.verifyMode ?? _1.VerifyMode.CHECKSUM,
                dataSensitivity: request.security?.dataSensitivity ?? _1.SecurityLevel.PRIVATE,
            },
            chunkHint: request.chunkHint ?? 0,
            priority: request.priority ?? _1.Priority.NORMAL,
            creditsOffered: 10,
            signature: new Uint8Array(64),
        };
        const plan = this.distributor.plan(taskRequest, negotiationResult.assignments, inputData, codeRef);
        ...
        const sessionKey = (0, _1.randomBytes)(32);
        const assembler = new src_1.ResultAssembler(plan, sessionKey);
        const taskHex = (0, _1.toHex)(negotiationResult.taskId);
        ...
        const remoteResult = await new Promise((resolve, reject) => {
            ...
            const timeoutHandle = setTimeout(() => {
                ...
                if (pending) {
                    ...
                    log.warn(`Task ${(0, _1.shortId)(negotiationResult.taskId)} timed out waiting for remote results ` +
            ...
            const chunkMonitors = new Map();
            const now = Date.now();
            for (let i = 0; i < plan.chunks.length; i++) {
                const chunkHex = (0, _1.toHex)(plan.chunks[i].chunkId);
            ...
            this.sendChunkDataToExecutors(plan, negotiationResult, wasmModule, moduleHash, sessionKey, entryPoint, deadline).catch((err) => {
     ...
     * ```
     */
    async run(fn, inputData, options = {}) {
        ...
        const packed = (0, src_1.packCodePayload)(language, codeBytes);
     ...
     */
    async sendChunkDataToExecutors(plan, negotiationResult, wasmModule, moduleHash, sessionKey, entryPoint, deadline) {
        for (let i = 0; i < plan.chunks.length; i++) {
            const chunk = plan.chunks[i];
            const assignment = negotiationResult.assignments[i % negotiationResult.assignments.length];
            // Encrypt the chunk payload with session key
            const encryptedPayload = plan.inputChunks[i] && plan.inputChunks[i].length > 0
                ? (0, _1.encrypt)(plan.inputChunks[i], sessionKey)
                : new Uint8Array(0);
            // Build wire format
            const wire = {
            ...
            const msg = (0, _1.encodeMessage)(_1.MessageType.CHUNK_DATA, (0, _1.encodeJSON)(wire));
            try {
                await this.transport.sendTo(assignment.peerAddress, msg);
                log.info(`CHUNK_DATA sent to ${assignment.peerAddress}: chunk ${(0, _1.shortId)(chunk.chunkId)} ` +
     ...
     */
    async executeLocally(wasmModule, inputData, entryPoint, startTime) {
        const { output, status } = await this.executionEngine.executeRaw(wasmModule, entryPoint, inputData, 30000);
        return {
            taskId: (0, _1.randomBytes)(16),
            data: output,
            totalTimeMs: Date.now() - startTime,
            chunksExecuted: 1,
            devicesUsed: 1,
            verified: status === _1.ChunkStatus.SUCCESS,
            localFallback: true,
        };
    }
    /**
     ...
     */
    async executeLocalMultiRuntime(language, code, inputData, timeoutMs) {
        const startTime = Date.now();
        try {
            const output = (0, src_1.executeMultiRuntime)(language, code, inputData, timeoutMs);
            return {
                taskId: (0, _1.randomBytes)(16),
                data: output,
                totalTimeMs: Date.now() - startTime,
    ...
    getPeers() {
        return this.peerTable.getActive().map((p) => {
            ...
            return {
                meshId: (0, _1.toHex)(p.meshId),
                shortId: (0, _1.shortId)(p.meshId),
                state: p.state,
                tier: p.tier,
     ...
     */
    meshIdHex() {
        return (0, _1.toHex)(this.discovery.getMeshId());
    }
    /**
     * Get this node's short ID (first 8 hex chars).
     */
    shortMeshId() {
        return (0, _1.shortId)(this.discovery.getMeshId());
    }
    /**
     ...
     */
    async computeCertified(wasmModule, inputData, options = {}) {
        return this.compute(wasmModule, inputData, {
            ...options,
            certify: true,
            verifyMode: _1.VerifyMode.REDUNDANT,
        });
    }
    /**
     ...
     */
    generateTaskCertificate(pending, completion, wasmModule, inputData, entryPoint) {
        ...
        for (const [_chunkHex, results] of allResults) {
            for (const result of results) {
                if (result.status !== _1.ChunkStatus.SUCCESS)
                    continue;
                ...
                attestationInputs.push({
                    ...
                    outputHash: (0, _1.hash256)(completion.result),
                });
            }
        }
        const certRequest = {
    ...
    handleTransportMessage(event) {
        if (!event.data)
            return;
        const msg = (0, _1.decodeMessage)(event.data);
        if (!msg)
            return;
        switch (msg.type) {
            case _1.MessageType.CHUNK_RESULT:
                this.handleChunkResult(msg.payload);
                break;
            case _1.MessageType.HEARTBEAT:
                this.handleHeartbeat(msg.payload);
                break;
            case _1.MessageType.DEPARTURE_NOTICE:
                this.handleDeparture(msg.payload);
                break;
            case _1.MessageType.CHECKPOINT_STORE:
     ...
     */
    handleChunkResult(payload) {
        const wire = (0, _1.decodeJSON)(payload);
        if (!wire) {
        ...
        const taskHex = (0, _1.toHex)(taskId);
        const chunkHex = (0, _1.toHex)(chunkId);
        log.info(`CHUNK_RESULT received: chunk ${(0, _1.shortId)(chunkId)} for task ${(0, _1.shortId)(taskId)}`, {
            status: _1.ChunkStatus[wire.status],
            executionTimeMs: wire.executionTimeMs,
            payloadSize: wire.encryptedPayload.length,
        });
        const pending = this.pendingTasks.get(taskHex);
        if (!pending) {
            log.warn(`No pending task for CHUNK_RESULT ${(0, _1.shortId)(taskId)} — may have timed out`);
            return;
        }
        ...
        if (wire.status === _1.ChunkStatus.FAILED) {
            ...
            if (wire.encryptedPayload.length > 0) {
                try {
                    const decrypted = (0, _1.decrypt)(new Uint8Array(wire.encryptedPayload), pending.sessionKey);
            ...
            log.warn(`Chunk ${(0, _1.shortId)(chunkId)} FAILED: ${errorMsg}`);
            this.ledger.recordFailure((0, _1.toHex)(new Uint8Array(wire.executorId)));
        ...
        const executorHex = (0, _1.toHex)(result.executorId);
        const myHex = this.meshIdHex();
        const taskIdStr = (0, _1.toHex)(taskId);
        ...
        if (result.status === _1.ChunkStatus.SUCCESS) {
            ...
            const chunk = pending.plan.chunks.find(c => (0, _1.toHex)(c.chunkId) === chunkHex);
        ...
        if (completion) {
            ...
            if (pending.heartbeatTimer)
                clearInterval(pending.heartbeatTimer);
            this.pendingTasks.delete(taskHex);
            log.info(`Task ${(0, _1.shortId)(taskId)} COMPLETE via remote execution`, {
            ...
            if (pending.certify && pending.inputData) {
                try {
                    certificate = this.generateTaskCertificate(pending, completion, pending.wasmModule, pending.inputData, pending.entryPoint);
                }
                catch (certErr) {
            ...
            log.debug(`Task ${(0, _1.shortId)(taskId)}: ${pending.resultsReceived}/${pending.expectedResults} results received`);
        }
    }
    ...
    handleHeartbeat(payload) {
        ...
        log.debug(`Heartbeat from ${(0, _1.shortId)(new Uint8Array(wire.executorId))} for chunk ${(0, _1.shortId)(chunkId)}`);
    }
    /**
     ...
     */
    handleDeparture(payload) {
        const wire = (0, _1.decodeJSON)(payload);
        if (!wire)
            return;
        const departingId = new Uint8Array(wire.meshId);
        const departingHex = (0, _1.toHex)(departingId);
        log.info(`Departure notice from ${(0, _1.shortId)(departingId)}`);
        ...
        for (const [taskHex, pending] of this.pendingTasks) {
            for (const [chunkHex, monitor] of pending.chunkMonitors) {
                ...
                const chunkIndex = pending.plan.chunks.findIndex(c => (0, _1.toHex)(c.chunkId) === chunkHex);
                const chunk = chunkIndex >= 0 ? pending.plan.chunks[chunkIndex] : undefined;
                if (chunk && (0, _1.toHex)(chunk.assigneeId) === departingHex) {
                    log.warn(`Executor ${(0, _1.shortId)(departingId)} departed, reassigning chunk ${(0, _1.shortId)(chunk.chunkId)}`);
                    monitor.dead = true;
                    pending.chunkMonitors.delete(chunkHex);
     ...
     */
    handleCheckpointStore(payload) {
        const wire = (0, _1.decodeJSON)(payload);
        if (!wire)
            return;
        const taskHex = (0, _1.toHex)(new Uint8Array(wire.taskId));
        const chunkHex = (0, _1.toHex)(new Uint8Array(wire.chunkId));
        const pending = this.pendingTasks.get(taskHex);
        if (!pending)
            return;
        // Store checkpoint (overwrite previous — only keep latest)
        if (!pending.checkpoints)
            pending.checkpoints = new Map();
    ...
    checkHeartbeats(taskHex) {
        ...
        for (const [chunkHex, monitor] of pending.chunkMonitors) {
            ...
            if (newMissed > monitor.missedBeats) {
                ...
                const chunkIndex = pending.plan.chunks.findIndex(c => (0, _1.toHex)(c.chunkId) === chunkHex);
                const chunk = chunkIndex >= 0 ? pending.plan.chunks[chunkIndex] : undefined;
                const chunkShort = chunk ? (0, _1.shortId)(chunk.chunkId) : chunkHex.substring(0, 8);
                if (monitor.missedBeats >= this.config.deadThreshold) {
                    ...
                    if (chunk) {
                        ...
                        this.ledger.recordFailure((0, _1.toHex)(chunk.assigneeId));
                        this.ledger.recordAvailability((0, _1.toHex)(chunk.assigneeId), false);
                        this.reassignDeadChunk(taskHex, chunk, pending, chunkIndex);
                    }
                }
     ...
     */
    async reassignDeadChunk(taskHex, chunk, pending, chunkIndex) {
        const chunkHex = (0, _1.toHex)(chunk.chunkId);
        const deadAssigneeHex = (0, _1.toHex)(chunk.assigneeId);
        // Find active peers excluding the dead executor
        const activePeers = this.peerTable.getActive().filter(p => (0, _1.toHex)(p.meshId) !== deadAssigneeHex && (0, _1.toHex)(p.meshId) !== this.meshIdHex());
        if (activePeers.length > 0) {
            ...
            if (newAddress) {
                log.info(`Reassigning chunk ${(0, _1.shortId)(chunk.chunkId)} to ${(0, _1.shortId)(newPeer.meshId)}`);
                try {
                    ...
                    const encryptedPayload = inputData && inputData.length > 0
                        ? (0, _1.encrypt)(inputData, pending.sessionKey)
                        : new Uint8Array(0);
                    ...
                    log.info(`Chunk ${(0, _1.shortId)(chunk.chunkId)} reassigned to ${(0, _1.shortId)(newPeer.meshId)} at ${newAddress}`);
                    ...
                    log.warn(`Reassignment to ${(0, _1.shortId)(newPeer.meshId)} failed: ${err.message}`);
                    // Fall through to local execution
                }
            }
        }
        // No peers available or reassignment failed — execute locally
        log.info(`No peers available for reassignment, executing chunk ${(0, _1.shortId)(chunk.chunkId)} locally`);
     ...
     */
    async executeDeadChunkLocally(taskHex, chunk, pending) {
        const chunkIndex = pending.plan.chunks.findIndex(c => (0, _1.toHex)(c.chunkId) === (0, _1.toHex)(chunk.chunkId));
        if (chunkIndex === -1)
            return;
        log.info(`Executing dead chunk ${(0, _1.shortId)(chunk.chunkId)} locally as fallback`);
        try {
            // Get the input data for this chunk
            const inputData = pending.plan.inputChunks[chunkIndex];
            if (!inputData || inputData.length === 0) {
                log.warn(`No input data for chunk ${(0, _1.shortId)(chunk.chunkId)}`);
                return;
            }
            // Encrypt input with session key (same as remote would)
            const encryptedPayload = (0, _1.encrypt)(inputData, pending.sessionKey);
            // Build a full CMPChunk for ExecutionEngine
            const fullChunk = {
                ...pending.plan.chunks[chunkIndex],
                payload: encryptedPayload,
            };
            const result = await this.executionEngine.executeChunk(fullChunk, pending.sessionKey, pending.wasmModule);
            log.info(`Local fallback for chunk ${(0, _1.shortId)(chunk.chunkId)}: ${_1.ChunkStatus[result.status]}`);
            // Feed to assembler
            const completion = pending.assembler.collectResult(result);
            pending.resultsReceived++;
            this.bus.emit('chunk:reassigned', {
            ...
            if (completion) {
                ...
                if (pending.heartbeatTimer)
                    clearInterval(pending.heartbeatTimer);
                this.pendingTasks.delete(taskHex);
                log.info(`Task ${(0, _1.shortId)(chunk.taskId)} COMPLETE (with local fallback)`, {
                    chunksExecuted: completion.chunksExecuted,
                    devicesUsed: completion.devicesUsed,
                    totalTimeMs: completion.totalTimeMs,
                });
            ...
            log.warn(`Local fallback execution failed for chunk ${(0, _1.shortId)(chunk.chunkId)}: ${err.message}`);
        }
    }
}
exports.CMPNode = CMPNode;
function buildCanonicalPayload(certId, codeHash, inputHash, outputHash, executionTimeMs) {
    const text = `CMP-CERT:${certId}:${(0, _1.toHex)(codeHash)}:${(0, _1.toHex)(inputHash)}:${(0, _1.toHex)(outputHash)}:${executionTimeMs}`;
    return (0, _1.hash256)(new TextEncoder().encode(text));
}
function hashCertificate(cert) {
    const parts = [
        cert.certId,
        (0, _1.toHex)(cert.codeHash),
        (0, _1.toHex)(cert.inputHash),
        (0, _1.toHex)(cert.outputHash),
        cert.deviceCount.toString(),
        cert.consensus.toString(),
        ...cert.attestations.map(a => (0, _1.toHex)(a.signature)),
    ];
    return (0, _1.hash256)(new TextEncoder().encode(parts.join(':')));
}
function certFromHex(hex) {
...
function generateCertificate(req) {
    const certId = `cmp-cert-${Date.now()}-${(0, _1.toHex)((0, _1.hash256)(new Uint8Array([
        ...req.requesterId, ...(0, _1.hash256)(req.wasmModule).slice(0, 8),
    ]))).substring(0, 12)}`;
    const codeHash = (0, _1.hash256)(req.wasmModule);
    const inputHash = (0, _1.hash256)(req.inputData);
    const outputHash = (0, _1.hash256)(req.outputData);
    const attestations = [];
    for (const att of req.attestations) {
        const canonicalPayload = buildCanonicalPayload(certId, codeHash, inputHash, att.outputHash, att.result.executionTimeMs);
        const signature = (0, _1.sign)(canonicalPayload, att.secretKey);
        attestations.push({
            meshId: att.meshId,
    ...
    const outputHashHex = (0, _1.toHex)(outputHash);
    const agreeingDevices = attestations.filter(a => (0, _1.toHex)(a.outputHash) === outputHashHex).length;
    ...
    for (const att of attestations) {
        ...
        if (!(0, _1.verify)(canonical, att.signature, att.publicKey)) {
    ...
    cert.requesterSignature = (0, _1.sign)(hashCertificate(cert), req.requesterSecretKey);
    return cert;
}
function verifyCertificate(cert) {
    ...
    for (let i = 0; i < cert.attestations.length; i++) {
        ...
        if ((0, _1.verify)(canonical, att.signature, att.publicKey)) {
            ...
            issues.push(`Invalid signature from device ${i + 1} (${(0, _1.toHex)(att.meshId).substring(0, 8)})`);
        }
    }
    const certOutputHex = (0, _1.toHex)(cert.outputHash);
    let agreeCount = 0;
    for (const att of cert.attestations) {
        if ((0, _1.toHex)(att.outputHash) === certOutputHex)
            agreeCount++;
        else
            issues.push(`Device ${(0, _1.toHex)(att.meshId).substring(0, 8)} produced different output`);
    }
    ...
    if (cert.outputData && cert.outputData.length > 0) {
        if ((0, _1.toHex)((0, _1.hash256)(cert.outputData)) !== certOutputHex) {
    ...
    const keySet = new Set(cert.attestations.map(a => (0, _1.toHex)(a.publicKey)));
...
function exportCertificateJSON(cert) {
    return JSON.stringify({
        version: cert.version, certId: cert.certId,
        codeHash: (0, _1.toHex)(cert.codeHash), entryPoint: cert.entryPoint,
        inputHash: (0, _1.toHex)(cert.inputHash), outputHash: (0, _1.toHex)(cert.outputHash),
        outputData: cert.outputData ? (0, _1.toHex)(cert.outputData) : undefined,
        ...
        attestations: cert.attestations.map(a => ({
            meshId: (0, _1.toHex)(a.meshId), publicKey: (0, _1.toHex)(a.publicKey),
            ...
            outputHash: (0, _1.toHex)(a.outputHash), completedAt: a.completedAt,
            signature: (0, _1.toHex)(a.signature),
        })),
        ...
        requesterId: (0, _1.toHex)(cert.requesterId), protocolVersion: cert.protocolVersion,
        requesterSignature: (0, _1.toHex)(cert.requesterSignature),
    }, null, 2);
}
function importCertificateJSON(json) {
    ...
    return {
        ...
        attestations: o.attestations.map((a) => ({
...
//# sourceMappingURL=cmp-node.js.map