...
class DreamScheduler {
    ...
    getCpuLoadFn;
    getActiveTasks;
    onPredictionsReady = null;
    constructor(getMERsFn, getCpuLoadFn, getActiveTasks, config) {
     ...
     */
    matchRealTask(taskType, moduleHash, inputFingerprint) {
        ...
        for (const pred of this.currentPredictions.predictions) {
            ...
            if (pred.taskType !== taskType)
                continue;
            // Module hash must match exactly
            if (toHex(pred.moduleHash) !== moduleHashHex)
                continue;
            // Time must be within predicted window
            if (now < pred.predictedWindowStart || now > pred.predictedWindowEnd)
                continue;
     ...
     */
    detectTemporalRecurrence(summaries) {
        const predictions = [];
        // Group by taskType
        const byType = this.groupByTaskType(summaries);
        for (const [taskType, mers] of byType) {
            if (mers.length < 3)
                continue; // Need at least 3 for interval analysis
            // Sort by timestamp
            ...
            if (cv < 0.3 && avgInterval > 1000) { // At least 1 second intervals
                ...
                predictions.push({
                    ...
                    createdAt: Date.now(),
                    patternType: precognition_1.PredictionPattern.TEMPORAL_RECURRENCE,
                });
            }
        }
        return predictions;
     ...
     */
    detectSequentialChain(summaries) {
        const predictions = [];
            ...
            for (const [to, entry] of targets) {
                ...
                if (probability > 0.7 && entry.count > 3) {
                    ...
                    predictions.push({
                        ...
                        createdAt: Date.now(),
                        patternType: precognition_1.PredictionPattern.SEQUENTIAL_CHAIN,
                    });
                }
            }
        }
     ...
     */
    detectPayloadSimilarity(summaries) {
        const predictions = [];
        // Group by moduleHash
        ...
        for (const [moduleHash, mers] of byModule) {
            ...
            predictions.push({
                ...
                sourceMerIds: sorted.slice(-5).map(m => m.merId),
                createdAt: now,
                patternType: precognition_1.PredictionPattern.PAYLOAD_SIMILARITY,
            });
        }
        return predictions;
    }
        ...
        for (const [taskType, mers] of byType) {
            ...
            if (ratio > 0.6) {
                ...
                predictions.push({
                    ...
                    createdAt: Date.now(),
                    patternType: precognition_1.PredictionPattern.DIURNAL_CYCLE,
                });
            }
        }
        return predictions;
    ...
    detectBurstPattern(summaries) {
        ...
        for (const [taskType, mers] of byType) {
            ...
            predictions.push({
                ...
                createdAt: now,
                patternType: precognition_1.PredictionPattern.BURST_PATTERN,
            });
        }
        return predictions;
    }
    ...
    onIdleTick() {
        if (!this.running)
            return;
        const mers = this.getMERsFn();
        if (mers.length < this.config.minMersForPrediction)
            return;
        if (!this.checkIdleState())
            return;
...
exports.DreamScheduler = DreamScheduler;
