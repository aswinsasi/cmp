...
export interface AdapterConfig {
  ...
  entryPoint?: string;
  /** Force a specific convention (skip auto-detect) */
  forceConvention?: WasmConvention;
}
...
class WASICapture {
    ...
    return {
      fd_write: (fd: number, iovs: number, iovsLen: number, nwritten: number): number => {
        ...
        for (let i = 0; i < iovsLen; i++) {
          ...
          if (fd === 1) this.stdout.push(new Uint8Array(chunk));
          else if (fd === 2) this.stderr.push(new Uint8Array(chunk));
          totalWritten += len;
        }
      ...
      proc_exit: (code: number): void => { this.exitCode = code; },
      ...
      random_get: (bufPtr: number, bufLen: number): number => {
        if (!this.memory) return -1;
        const u8 = new Uint8Array(this.memory.buffer);
        for (let i = 0; i < bufLen; i++) u8[bufPtr + i] = Math.floor(Math.random() * 256);
        return 0;
      },
      sched_yield: (): number => 0,
      path_open: (): number => 44, // ENOSYS
    };
  }
  ...
  getStdout(): Uint8Array {
    ...
    let offset = 0;
    for (const chunk of this.stdout) { result.set(chunk, offset); offset += chunk.length; }
    return result;
  }
}
...
export class WasmAdapter {
  ...
  private stats = {
    executions: 0,
    byConvention: new Map<WasmConvention, number>(),
  };
   ...
   */
  async detectConvention(wasmBinary: Uint8Array): Promise<{
    convention: WasmConvention;
    exports: string[];
    hasMemory: boolean;
    hasMalloc: boolean;
  ...
  async execute(
    ...
    try {
      ...
      let result: { output: Uint8Array; memoryUsed: number };
  ...
  private async executeCMPNative(
    wasmBinary: Uint8Array, input: Uint8Array, entryPoint: string,
  ): Promise<{ output: Uint8Array; memoryUsed: number }> {
    const module = await WebAssembly.compile(wasmBinary);
    ...
    const fn = instance.exports[entryPoint] as Function;
    if (!fn) throw new Error(`Export "${entryPoint}" not found`);

    const outputLen = fn(input.length) as number;
  ...
  private async executeCMPRuntime(
    wasmBinary: Uint8Array, input: Uint8Array, entryPoint: string,
  ): Promise<{ output: Uint8Array; memoryUsed: number }> {
    const module = await WebAssembly.compile(wasmBinary);
    const memory = new WebAssembly.Memory({
    ...
    if (wasmMemory.buffer.byteLength < neededBytes) {
      ...
      try { wasmMemory.grow(pages); } catch { /* proceed with current */ }
    }
    ...
    const outputLen = fn(0, input.length) as number;
  ...
  private async executeAllocator(
    wasmBinary: Uint8Array, input: Uint8Array, entryPoint: string,
  ): Promise<{ output: Uint8Array; memoryUsed: number }> {
    const module = await WebAssembly.compile(wasmBinary);
    ...
    const malloc = exports.malloc as Function;
    const free = exports.free as Function;
    const fn = exports[entryPoint] as Function;

    if (!malloc || !free) throw new Error('malloc/free not found');
    if (!fn) throw new Error(`Export "${entryPoint}" not found`);

    // Allocate input buffer
    const inputPtr = malloc(input.length) as number;
    const view = new Uint8Array(wasmMemory.buffer);
    ...
    const result = fn(inputPtr, input.length) as number;
  ...
  private async executeWASICommand(
    wasmBinary: Uint8Array, input: Uint8Array,
  ): Promise<{ output: Uint8Array; memoryUsed: number }> {
    const module = await WebAssembly.compile(wasmBinary);
  ...
  private async executeWASIReactor(
    wasmBinary: Uint8Array, input: Uint8Array, entryPoint: string,
  ): Promise<{ output: Uint8Array; memoryUsed: number }> {
    const module = await WebAssembly.compile(wasmBinary);
    const wasi = new WASICapture();
    ...
    // Initialize
    const init = instance.exports._initialize as Function;
    if (init) init();

    // Now call the entry point like a regular function
    const fn = instance.exports[entryPoint] as Function;
    if (!fn) throw new Error(`Export "${entryPoint}" not found in WASI reactor`);
    ...
    if (malloc) {
      ...
      const outputLen = fn(inputPtr, input.length) as number;
  ...
  getStats(): { executions: number; byConvention: Record<string, number> } {
    const conv: Record<string, number> = {};
    for (const [k, v] of this.stats.byConvention) conv[k] = v;
