...
exports.IncentiveLedger = void 0;
const incentive_1 = require("../types/incentive");
const crypto_1 = require("../crypto");
const logger_1 = require("../utils/logger");
const log = new logger_1.Logger('Ledger');
const DEFAULT_LEDGER_CONFIG = {
    bootstrapCredits: incentive_1.BOOTSTRAP_CREDITS,
    decayRate: incentive_1.REPUTATION_DECAY_RATE,
    minReputation: 500,
};
class IncentiveLedger {
     ...
     */
    spendCredits(meshIdHex, amount, taskId, chunkId = '') {
     ...
     */
    earnCredits(executorHex, requesterHex, amount, taskId, chunkId) {
        ...
        if (this.config.signingKey) {
            const txData = new TextEncoder().encode(`${tx.id}|${tx.fromId}|${tx.toId}|${tx.amount}|${tx.taskId}|${tx.chunkId}|${tx.timestamp}`);
            tx.fromSignature = (0, crypto_1.sign)(txData, this.config.signingKey);
        }
     ...
     */
    recordCompletion(executorHex, accurate) {
        ...
        account.factors.accuracyRate = this.ema(account.factors.accuracyRate, accurate ? 1.0 : 0.0, total);
     ...
     */
    recordAvailability(meshIdHex, wasAvailable) {
        const account = this.getAccount(meshIdHex);
        const total = account.tasksCompleted + account.tasksFailed + 1;
        account.factors.availabilityRate = this.ema(account.factors.availabilityRate, wasAvailable ? 1.0 : 0.0, total);
        this.recalculateReputation(account);
    }
    /**
     * Record resource honesty (did the node deliver what it advertised).
     */
    recordResourceHonesty(meshIdHex, honest) {
        const account = this.getAccount(meshIdHex);
        const total = account.tasksCompleted + account.tasksFailed + 1;
        account.factors.resourceHonesty = this.ema(account.factors.resourceHonesty, honest ? 1.0 : 0.0, total);
        this.recalculateReputation(account);
    }
    /**
    ...
    applyDecay() {
        ...
        for (const account of this.accounts.values()) {
            ...
            if (weeksInactive >= 1) {
                ...
                account.reputation = Math.max(incentive_1.REPUTATION_MIN, Math.round(account.reputation * factor));
     ...
     */
    recalculateReputation(account) {
        const w = incentive_1.REPUTATION_WEIGHTS;
        const f = account.factors;
        const weighted = f.completionRate * w.completionRate +
            f.accuracyRate * w.accuracyRate +
            f.availabilityRate * w.availabilityRate +
            f.resourceHonesty * w.resourceHonesty;
        const oldRep = account.reputation;
        account.reputation = Math.round(Math.max(incentive_1.REPUTATION_MIN, Math.min(incentive_1.REPUTATION_MAX, weighted * incentive_1.REPUTATION_MAX)));
        if (account.reputation !== oldRep) {
