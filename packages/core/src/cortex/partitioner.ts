...
export function partitionModel(
  ...
  maxPartitionsPerDevice: number = 4,
): PartitionAssignment[] {
  if (devices.length === 0) throw new Error('No devices available for partitioning');
  if (manifest.totalLayers === 0) throw new Error('Model has no layers');
  ...
  if (totalAvailable < manifest.totalSizeBytes) {
    throw new Error(
      `Insufficient mesh memory: need ${manifest.totalSizeBytes} bytes, ` +
...
export function rebalancePartitions(
  ...
  newDevices: DeviceCapacity[],
): { assignments: PartitionAssignment[]; migrations: Array<{ from: string; to: string; layerRange: [number, number] }> } {
  // Simple strategy: re-partition from scratch, then compute diff
  const newAssignments = partitionModel(manifest, newDevices);

  const migrations: Array<{ from: string; to: string; layerRange: [number, number] }> = [];
  ...
  for (const a of currentAssignments) {
    for (let l = a.layerRange[0]; l <= a.layerRange[1]; l++) {
      if (!currentMap.has(a.device)) currentMap.set(a.device, new Set());
  ...
  for (const newA of newAssignments) {
    for (let l = newA.layerRange[0]; l <= newA.layerRange[1]; l++) {
      ...
      for (const [dev, layers] of currentMap) {
        if (layers.has(l)) { currentDevice = dev; break; }
      }
...
function compactMigrations(
  migrations: Array<{ from: string; to: string; layerRange: [number, number] }>
): Array<{ from: string; to: string; layerRange: [number, number] }> {
  ...
  const result: typeof migrations = [sorted[0]];
