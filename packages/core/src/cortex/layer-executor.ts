...
import type { Tensor, LayerPartition } from '../types/cortex';

// ─── Layer Function ───

export type LayerFunction = (input: Tensor, layerIndex: number, weights: Float32Array) => Tensor;
 ...
 */
export function defaultLayerFunction(input: Tensor, layerIndex: number, weights: Float32Array): Tensor {
...
export class LayerExecutor {
  ...
  readonly layerRange: [number, number];
  private weights: Float32Array;
  private layerFn: LayerFunction;

  /** Stats */
  private executionCount = 0;
  private totalComputeMs = 0;

  constructor(partition: LayerPartition, layerFn?: LayerFunction) {
   ...
   */
  execute(input: Tensor): { output: Tensor; computeMs: number } {
   ...
   * Get execution stats.
   */
  getStats(): { executionCount: number; totalComputeMs: number; avgComputeMs: number } {
    return {
