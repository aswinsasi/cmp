...
class QuorumSensor {
     ...
     */
    observe(signalType, strength = 1.0, evidence) {
        const observation = {
            signalType,
     ...
     */
    receiveObservation(wire) {
        // Don't re-add our own observations
        if (wire.observerId === this.deviceId)
            return;
        const observation = {
    ...
    getState(signalType) {
        ...
        const validObs = allObs.filter(o => (now - o.observedAt) <= windowMs && o.strength >= minStrength);
...
exports.QuorumSensor = QuorumSensor;
//# sourceMappingURL=quorum-sensor.js.map