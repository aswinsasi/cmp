...
exports.packCodePayload = packCodePayload;
exports.detectRuntime = detectRuntime;
exports.executeJavaScript = executeJavaScript;
exports.executeSubprocess = executeSubprocess;
exports.executePython = executePython;
exports.executeMultiRuntime = executeMultiRuntime;
exports.isRuntimeAvailable = isRuntimeAvailable;
exports.listAvailableRuntimes = listAvailableRuntimes;
exports.listAllLanguages = listAllLanguages;
 ...
 */
function packCodePayload(runtime, code) {
 ...
 */
function detectRuntime(moduleBytes) {
    if (moduleBytes.length < 4)
        return null;
 ...
 */
function executeJavaScript(code, input, timeoutMs = 10000) {
    const sandbox = {
 ...
 */
function executeSubprocess(lang, code, input, timeoutMs = 30000) {
    ...
    try {
        ...
        if (lang.mode === 'compile' && lang.compileCmd) {
            ...
            try {
                (0, child_process_1.execFileSync)(compileArgs[0], compileArgs.slice(1), {
                    timeout: timeoutMs, encoding: 'utf8', stdio: 'pipe',
                });
                stdout = (0, child_process_1.execFileSync)(outFile, [], {
                    timeout: timeoutMs, encoding: 'utf8', stdio: 'pipe',
                });
            }
            catch (execErr) {
            ...
            try {
                stdout = (0, child_process_1.execFileSync)(cmd, ['run', srcFile], {
            ...
            try {
                stdout = (0, child_process_1.execFileSync)(cmd, [srcFile], {
            ...
            try {
                stdout = (0, child_process_1.execFileSync)(cmd, runArgs, {
                    timeout: timeoutMs,
                    encoding: 'utf8',
 ...
 */
function executePython(code, input, timeoutMs = 30000) {
    return executeSubprocess(LANGUAGES.python, code, input, timeoutMs);
}
/**
 * Execute code in any supported runtime.
 */
function executeMultiRuntime(runtime, code, input, timeoutMs = 10000) {
...
function isRuntimeAvailable(runtime) {
    if (runtime === 'javascript' || runtime === 'wasm')
        return true;
    const lang = LANGUAGES[runtime];
    if (!lang)
        return false;
...
function findCommand(candidates) {
    ...
    if (_commandCache.has(key))
        return _commandCache.get(key);
    for (const cmd of candidates) {
        try {
            if (process.platform === 'win32') {
                (0, child_process_1.execSync)(`where ${cmd}`, { encoding: 'utf8', stdio: 'pipe', timeout: 5000 });
            }
            else {
                (0, child_process_1.execSync)(`which ${cmd}`, { encoding: 'utf8', stdio: 'pipe', timeout: 5000 });
            }
...
//# sourceMappingURL=multi-runtime.js.map